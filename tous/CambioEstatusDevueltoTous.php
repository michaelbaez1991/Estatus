<?php 
	...
	//XML GETORDERS
		...
		for ($row = 2; $row <= $lastRow; $row++) {
	        $orden = $worksheet->getCell('A'.$row)->getValue();
	        $ref = $worksheet->getCell('B'.$row)->getValue();

	        $query = "UPDATE [KRONO].[dbo].[TEMP-ORDENES_API_LINIO_ITEMS] SET [Status_] = 'Devuelto' WHERE portal = 'TOUS' AND OrderId = '00$orden-TOUS' AND Sku = '$ref'";
