		...
		for ($row = 2; $row <= $lastRow; $row++) {
		    ...
		    foreach ($productos as $value) {
		        foreach ($value as $value1) {
		            if($factura != ''){
				        if ($guia != '') {
				        	$query = "UPDATE [KRONO].[dbo].[TEMP-ORDENES_API_LINIO_ITEMS] SET [Status_] = 'Facturado', [NMRFCT] = '$factura', [TrackingCode] = '$guia' WHERE portal = 'REVIVE' AND OrderId = '$value1' AND Sku = '$ref'";
				        	$consulta = $db->query($query); 
				        	print_r($consulta);
				        }else{
				        	$query = "UPDATE [KRONO].[dbo].[TEMP-ORDENES_API_LINIO_ITEMS] SET [Status_] = 'Facturado', [NMRFCT] = '$factura' WHERE portal = 'REVIVE' AND OrderId = '$value1' AND Sku = '$ref'";
				        	$consulta = $db->query($query); 
				        	print_r($consulta);
				        }
			        }else{
			        	 if ($guia != '') {
				        	...
				        	$query = "UPDATE [KRONO].[dbo].[TEMP-ORDENES_API_LINIO_ITEMS] SET [Status_] = 'Facturado' WHERE portal = 'REVIVE' AND OrderId = '$value1' AND Sku = '$ref'";
				        	$consulta = $db->query($query); 
				        	print_r($consulta);
				        }
			        }
